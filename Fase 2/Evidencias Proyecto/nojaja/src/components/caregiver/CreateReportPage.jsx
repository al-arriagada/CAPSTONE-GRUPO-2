...
export default function CreateReportPage() {
  ...
  // 'routineActivities' ahora guardará las *alertas* de hoy
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // 1. Obtener las ALERTAS de hoy (la verdadera lista de "tareas")
      //    Incluimos los detalles de la rutina anidada
      const { data: todayAlerts, error: alertError } = await supabase
        ...
        `)
        .eq("pet_id", petId)
        .gte("scheduled_at", startOfDayUTC) // >= Inicio del día en UTC
        .lt("scheduled_at", endOfDayUTC)   // < Inicio del día siguiente en UTC
        .order("scheduled_at", { ascending: true });

      if (alertError) throw alertError;

      // 2. Obtener los LOGS de hoy (lo que ya se registró)
      ...
      // 3. Mapear los logs para unirlos fácilmente
      const logMap = new Map();
      for (const log of (todayLogs || [])) {
        logMap.set(log.routine_id, log);
      }
      
      // 4. Combinar Alertas y Logs
      const initialActivities = [];
      const initialChanges = {};

      for (const alert of (todayAlerts || [])) {
        if (!alert.routine) continue; // Si la rutina fue borrada, la saltamos

        const log = logMap.get(alert.routine_id);
        
        // La actividad que se mostrará en la UI
        initialActivities.push({
          routine_id: alert.routine_id,
          alert_id: alert.alert_id,
          // Detalles de la rutina anidada
        ...
        // El estado inicial (lo que diga el LOG, o si no, lo que diga la ALERTA)
        initialChanges[alert.routine_id] = {
          status_id: log ? log.status_id : alert.status_id,
          notes: log ? (log.notes || '') : '', // Solo las notas del log
        };
      }
  ...
  // --- Manejar Cambios (CORREGIDO para 'skipped') ---
  const handleLogChange = (routineId, field, value) => {
    setActivityChanges(prevChanges => {
      ...
      if (field === 'status_id') { 
        const completionTimestamp = currentChange.completed_at || new Date().toISOString();
        ...
        if (value === 'completed') {
          ...
          newChangeData.notes = currentChange.notes || '';
        } else if (value === null || !value) { // 'scheduled' o null
          newChangeData.completed_at = null;
          newChangeData.completed_by = null;
          newChangeData.notes = ''; // Limpia las notas al deseleccionar
          newChangeData.status_id = null; // O 'scheduled' si prefieres
        }
      }
  ...
  // --- Enviar Reporte (CORREGIDO para 'skipped' y 'activities') ---
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      // Itera sobre las ACTIVIDADES DE HOY (que vienen de las alertas)
      for (const activity of activities) {
        const routineId = activity.routine_id;
        const change = activityChanges[routineId];

        // Si no hay cambio o no hay estado, no se guarda nada
        if (!change || !change.status_id) { 
          continue;
        }

        // (La lógica 'hasChanged' se movió al trigger,
        // aquí simplemente enviamos todo lo que tenga un estado)
        logsToUpsert.push({
          ...
          pet_id: petId,
          user_id: user.id, 
          status_id: change.status_id, // 'completed' o 'skipped'
          notes: change.notes || null,
        });
      }
      
      if (logsToUpsert.length > 0) {
        console.log("Guardando logs:", logsToUpsert);
        ...
        console.log("No hay cambios en los logs para guardar.");
      }

      alert("¡Reporte guardado con éxito!");
      // Redirige a donde el usuario vino, o a un 'fallback'
      ...
      {!loading && !error && (
        <div className="space-y-6">
          ...
          {activities.length > 0 ? (
            ...
            <p className="text-gray-500 italic">No hay actividades programadas para hoy.</p>
          )}
...
function ActivityLogCard({ activity, log, onChange }) {
  ...
  const isOmitted = currentStatus === 'skipped'; // Usa 'skipped'
    ...
    <div className={`p-4 rounded-lg border ${
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
           ...
           <button title="Omitida" onClick={() => handleStatusChange('skipped')} className={`p-1 rounded-full ${isOmitted ? 'bg-red-600 text-white ring-2 ring-red-300' : 'bg-gray-200 text-gray-500 hover:bg-red-100'}`}> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg> </button> {/* Usa 'skipped' */}
        </div>
      </div>
